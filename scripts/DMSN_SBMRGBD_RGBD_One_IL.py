...
from DMSN_module_RGBD_One_IL import DMSN_module
...
def getData(train_dir, dataset_dir, scene, dataset_dir_depth):
    ...
    for i in range(0, len(X_list)):
        ...
        x = kImage.load_img(X_list_depth[i], grayscale = True)
        x = kImage.img_to_array(x)
        Depth.append(x)
    ...
    return[X, Y, cls_weight_list, Depth]
...
def train(data, scene, mdl_path, vgg_weights_path):
    ...
    model = DMSN_module(lr, img_shape, scene, vgg_weights_path)
    ...
    model.fit([data[0], data[3]], data[1], 
...
main_dir = os.path.join('..', 'DMSN_One_IL')
...
main_mdl_dir = os.path.join(main_dir, 'SBMRGBD', 'models')
...
for scene in dataset:
    ...
    train_dir = os.path.join('..', 'datasets','SBMRGBD_dataset', scene, 'train', 'GT')
    dataset_dir = os.path.join('..', 'datasets','SBMRGBD_dataset', scene, 'train', 'RGB')
    dataset_dir_depth = os.path.join('..', 'datasets','SBMRGBD_dataset', scene, 'train', 'DEPTH')

    mdl_path = os.path.join(main_mdl_dir, 'mdl_SBMRGBD_RGBD_One_IL_'+ str(scene) + '.h5')
